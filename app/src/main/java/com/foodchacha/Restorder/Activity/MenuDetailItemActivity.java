...
import com.foodchacha.Restorder.Adapter.CommentAdapter;
import com.foodchacha.Restorder.Adapter.MenuViewPagerAdapter;
...
public class MenuDetailItemActivity extends AppCompatActivity implements View.OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		llBottomChechout = findViewById(R.id.llBottomChechout);
		Button buttonCheckout = (Button)findViewById(R.id.btnCheckout);
		Button buttonShopping = (Button)findViewById(R.id.btnShopping);
		buttonCheckout.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				Intent inCheckout = new Intent(getApplicationContext(),CheckoutActivity.class);
				startActivity(inCheckout);
			}
		});
	...
	public void onClick(View v) {
		...
		switch (id) {
			...
			case R.id.ivPlus:
				...
				if(count > 0)
				{
					llBottomChechout.setVisibility(View.VISIBLE);
				}else
				{
					llBottomChechout.setVisibility(View.VISIBLE);
				}
			...
			case R.id.ivMinus:
				...
				if (count != 0) {
					...
					}else
					{
						...
						llBottomChechout.setVisibility(View.GONE);
					}
				...
				if (response.isSuccessful()) {
					...
					if (response.body().response.code == Constant.Response_OK) {
						if (response.body().menu_list_cat != null) {
							...
							if(Integer.parseInt(response.body().menu_list_cat.menu_qty) > 0)
							{
								llBottomChechout.setVisibility(View.VISIBLE);
							}else
							{
